...
float* bucketSort(float* data, int proc, int nPerProc, int buckPerProc)
{
    ...
    omp_lock_t write_locks[numberOfBuckets];
    for(int i=0; i<numberOfBuckets; i++) {
        omp_init_lock(&write_locks[i]);
    }
 
    for (int i = 0; i < numberOfBuckets; i++) {
        buckets[i] = new float[int(2 * n / numberOfBuckets)];
        bucketsIndexes[i] = 0;
    }
    ...
    for (int i = 0; i < n; i++)
    {
        int bi = numberOfBuckets * data[i];
        if (bi == numberOfBuckets) {
            bi--; 
        }
        omp_set_lock(&write_locks[bi]);
        buckets[bi][bucketsIndexes[bi]] = data[i];
        bucketsIndexes[bi]++;
        omp_unset_lock(&write_locks[bi]);
